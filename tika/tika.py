...
if(hasattr(sys, '_MEIPASS')):
    TikaFilesPath = os.path.join(sys._MEIPASS,'data_files')
    TikaJarPath = os.getenv('TIKA_PATH', os.path.join(sys._MEIPASS,'data_files'))
    log_path = os.getenv('TIKA_LOG_PATH', os.path.join(sys._MEIPASS,'data_files'))
else:
...
def checkTikaServer(scheme="http", serverHost=ServerHost, port=Port, tikaServerJar=TikaServerJar, classpath=None, config_path=None):
    ...
    # jarPath = os.path.join(sys._MEIPASS,'data_files', 'tika-server.jar')
